...
class EmotionClassifier(nn.Module):
    """
    Attention-pooling emotion classification head.

    Input:  Transformer hidden states (B, T, d_model)
    Output: (B, num_emotions) logits

    Architecture:
        1. Learned attention query → weighted average over time frames
        2. LayerNorm + dropout
        3. Two-layer FFN with GELU
        4. Linear projection to num_emotions
    """

    def __init__(self, d_model: int, num_emotions: int, dropout: float = 0.15):
        super().__init__()
        self.num_emotions = num_emotions

        # Attention pooling: learn a query vector that attends over time
    ...
    def forward(
        ...
        returns       : (B, num_emotions) logits
        """
        # Project hidden states for attention scoring
        projected = self.attn_proj(hidden_states)        # (B, T, d_model)

        # Compute attention scores: dot product with learned query
...
class MimiHuBERTBridge(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.num_emotions  = m.get("num_emotions", None) or 0
    ...
    def forward(
        ...
        returns: tuple of:
            features       : (B, 4T, output_dim) — HuBERT-like features
            present_kvs    : KV-cache list or None
            emotion_logits : (B, num_emotions) or None
        """
        ...
        # Emotion classification (from transformer hidden states, before output proj)
        emotion_logits = None
        if (self.emotion_head is not None
                and return_emotion
                and not use_cache):  # skip during streaming
            emotion_logits = self.emotion_head(x)  # (B, num_emotions)

        # Output projection
        features = self.output_proj(x)       # (B, 4T, output_dim)

        return features, present_kvs, emotion_logits
