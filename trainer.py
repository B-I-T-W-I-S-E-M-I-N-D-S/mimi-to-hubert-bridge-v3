...
from emotion_dataset import (
    build_emotion_dataloaders, EmotionMimiHuBERTDataset,
    emotion_collate_fn, EMOTION_CLASSES, IDX_TO_EMOTION,
)
...
class Trainer:
    ...
    def __init__(self, cfg: dict):
        ...
        # ── Emotion flag ──────────────────────────────────────────────────────
        self.use_emotion = (cfg["model"].get("num_emotions", 0) or 0) > 0
        self.emotion_classes = EMOTION_CLASSES if self.use_emotion else []
        ...
        if self.is_main:
            ...
            if self.use_emotion:
                logger.info(
                    f"Emotion classification: ENABLED ({cfg['model']['num_emotions']} classes)"
                )
    ...
    def _build_distributed_loaders(self, cfg: dict):
        ...
        # ── Detect emotion CSV → use emotion dataset ──────────────────────────
        use_emotion = (cfg["model"].get("num_emotions", 0) or 0) > 0
        emotion_csv = d_cfg.get("emotion_csv", None)

        if use_emotion and emotion_csv and Path(emotion_csv).exists():
            if self.is_main:
            ...
            if (self.criterion.emotion is not None
                    and cfg["training"].get("emotion", {}).get("class_weights") == "auto"):
                self.criterion.emotion.set_class_weights(class_counts)
                if self.is_main:
        ...
        if self.is_main:
            logger.info("Using original JSONL dataset (no emotion_csv found)")
    ...
    def _train_step(self, batch: dict, is_accum_step: bool) -> dict:
        ...
        if use_adv and not is_accum_step:
            ...
            with autocast(device_type=self.amp_device, enabled=self.mixed_precision):
                pred_det, _, _ = self.bridge(tokens, return_emotion=False)
    ...
    def _val_epoch(self) -> dict:
        ...
        for batch in self.val_loader:
            ...
            with autocast(device_type=self.amp_device, enabled=self.mixed_precision):
                pred, _, emotion_logits = self.bridge(batch["tokens"])

            pred_fp32   = pred.float()
            target_fp32 = batch["hubert"].float()
            _, logs     = self.criterion(
                pred_fp32, target_fp32, batch,
                emotion_logits=emotion_logits,
            )
            ...
            # Emotion accuracy for validation logging
            if (emotion_logits is not None
                    and "emotion_labels" in batch
                    and batch["emotion_labels"] is not None):
                emo_preds = emotion_logits.argmax(dim=-1)
                emo_acc = (emo_preds == batch["emotion_labels"]).float().mean().item()
                logs["val_emotion_acc"] = emo_acc
