...
===========================================
Implements all loss components for Mimi-to-HuBERT bridge training:
  - Reconstruction (MSE)
  - CTC / ASR consistency
...
class ReconstructionLoss(nn.Module):
    """Primary MSE loss against HuBERT target features."""
...
class CTCConsistencyLoss(nn.Module):
    ...
    def __init__(self, input_dim: int = 1024, vocab_size: int = 31):
        super().__init__()
...
class AlignmentLoss(nn.Module):
    ...
    def __init__(self, input_dim: int = 1024, num_phones: int = 40):
        super().__init__()
...
# ──────────────────────────────────────────────────────────────────────────────
# 8. Emotion Classification Loss
# ──────────────────────────────────────────────────────────────────────────────

class EmotionLoss(nn.Module):
    """
    Focal-loss variant for emotion classification.

    Handles class imbalance via:
      1. Per-class weights (inverse frequency) — set via set_class_weights()
      2. Focal loss (γ=2) to down-weight easy examples and focus on hard ones
      3. Optional label smoothing (ε=0.1)
    ...
    def __init__(
        ...
        # Per-class weights (default uniform; updated by set_class_weights())
        self.register_buffer("class_weights", torch.ones(num_emotions))

    def set_class_weights(self, counts: torch.Tensor):
        """
        Compute inverse-frequency weights from per-class sample counts.
        counts : (num_emotions,) int tensor — number of samples per class
        """
        total = counts.sum().float()
        weights = total / (self.num_emotions * counts.float().clamp(min=1))
    ...
    def forward(
        ...
        """
        logits : (B, num_emotions) raw logits
        labels : (B,) int64 class indices
        """
        # Label smoothing: soft targets
        B = logits.size(0)
        eps = self.label_smoothing
        n_cls = self.num_emotions

        # One-hot encode + smooth
        smooth_targets = torch.full_like(logits, eps / (n_cls - 1))
        smooth_targets.scatter_(1, labels.unsqueeze(1), 1.0 - eps)
        ...
        # Apply class weights
        sample_weights = self.class_weights.to(logits.device)[labels]  # (B,)

        # Combine focal + class weights
        loss = (focal_weight * sample_weights * ce).mean()

        # Compute accuracy for logging
        preds = logits.argmax(dim=-1)
        acc = (preds == labels).float().mean().item()

        return loss, {"emotion_loss": loss.item(), "emotion_acc": acc}
...
class BridgeLoss(nn.Module):
    ...
    with the correct output_dim (e.g. 1024 for HuBERT-large) read from cfg.
    """
    ...
    def __init__(self, cfg: dict):
        ...
        num_emotions = cfg["model"].get("num_emotions", None) or 0
        ...
        # Emotion loss (optional — only when model has emotion head)
        self.emotion = None
        if num_emotions > 0 and w.get("emotion", 0) > 0:
            emo_cfg = cfg["training"].get("emotion", {})
            self.emotion = EmotionLoss(
                num_emotions=num_emotions,
                focal_gamma=emo_cfg.get("focal_gamma", 2.0),
                label_smoothing=emo_cfg.get("label_smoothing", 0.1),
            )
    ...
    def forward(
        ...
        if (self.weights.get("emotion", 0) > 0
                and self.emotion is not None
                and emotion_logits is not None
                and "emotion_labels" in batch
                and batch["emotion_labels"] is not None):
            l, d = self.emotion(emotion_logits, batch["emotion_labels"])
            total = total + self.weights["emotion"] * l
            logs.update(d)
